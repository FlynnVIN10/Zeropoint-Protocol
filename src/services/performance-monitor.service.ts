...
export class PerformanceMonitorService {
   ...
   */
  updateCacheMetrics(hitRate: number, evictionRate: number, memoryUsage: number): void {
  ...
  checkPerformanceTargets(): {
    ...
    const slowRequestRate = (this.metrics.responseTime.slowRequests / this.totalRequests) * 100;
    
    return {
      responseTime: this.metrics.responseTime.average <= performanceTargets.responseTime.target,
      ...
      concurrentLoad: this.metrics.concurrentLoad.current <= performanceTargets.concurrentLoad.target,
    };
  }
...
Cache Hit Rate: ${this.metrics.cache.hitRate.toFixed(2)}%
  ...
  private calculatePercentiles(): void {
    ...
    // Calculate average
    this.metrics.responseTime.average = sorted.reduce((a, b) => a + b, 0) / total;
  ...
  private checkResponseTimeAlerts(responseTime: number): void {
    if (responseTime > performanceTargets.responseTime.critical) {
      this.createAlert('critical', 'Response time exceeded critical threshold', 'responseTime', responseTime, performanceTargets.responseTime.critical);
    } else if (responseTime > performanceTargets.responseTime.warning) {
      this.createAlert('warning', 'Response time exceeded warning threshold', 'responseTime', responseTime, performanceTargets.responseTime.warning);
    }
  }

  private checkErrorRateAlerts(): void {
    if (this.metrics.errors.rate > 5) { // 5% error rate threshold
      this.createAlert('critical', 'Error rate exceeded 5% threshold', 'errorRate', this.metrics.errors.rate, 5);
    }
  }

  private checkConcurrentLoadAlerts(currentLoad: number): void {
    if (currentLoad > performanceTargets.concurrentLoad.critical) {
      this.createAlert('critical', 'Concurrent load exceeded critical threshold', 'concurrentLoad', currentLoad, performanceTargets.concurrentLoad.critical);
    } else if (currentLoad > performanceTargets.concurrentLoad.warning) {
      this.createAlert('warning', 'Concurrent load exceeded warning threshold', 'concurrentLoad', currentLoad, performanceTargets.concurrentLoad.warning);
    }
  }

  private checkCacheAlerts(evictionRate: number): void {
    if (evictionRate > performanceTargets.redisHealthCheckConfig.maxEvictionRate) {
      this.createAlert('warning', 'Cache eviction rate exceeded threshold', 'evictionRate', evictionRate, performanceTargets.redisHealthCheckConfig.maxEvictionRate);
    }
  }

  private createAlert(level: 'info' | 'warning' | 'critical', message: string, metric: string, value: number, threshold: number): void {
    ...
    if (level === 'critical') {
      this.logger.error(`CRITICAL ALERT: ${message} - ${metric}: ${value} (threshold: ${threshold})`);
    } else if (level === 'warning') {
      this.logger.warn(`WARNING: ${message} - ${metric}: ${value} (threshold: ${threshold})`);
    }
  }
