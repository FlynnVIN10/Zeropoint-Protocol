...
describe('Gateway E2E Tests (Phase 2)', () => {
  ...
  describe('Core Gateway Endpoints', () => {
    ...
    it('/v1/status (GET) - should return gateway status', () => {
      return request(app.getHttpServer())
        ...
        .expect(200)
        .expect((res) => {
          expect(res.body).toHaveProperty('service', 'Zeropoint Protocol API Gateway');
  ...
  describe('Python Backend Integration', () => {
    it('/v1/generate-text (POST) - should forward to Python backend', () => {
      const mockResponse = { data: { generated_text: 'Test response from Python backend' } };
    ...
    it('/v1/generate-image (POST) - should forward to Python backend', () => {
      const mockResponse = { data: { image_url: 'test-image.jpg', cid: 'test-cid' } };
      mockHttpService.post.mockReturnValue(of(mockResponse));
    ...
    it('/v1/generate-code (POST) - should forward to Python backend', () => {
      const mockResponse = { data: { code: 'console.log("Hello, World!");', language: 'javascript' } };
      mockHttpService.post.mockReturnValue(of(mockResponse));
    ...
    it('should handle Python backend errors gracefully', () => {
      mockHttpService.post.mockReturnValue(throwError(() => new Error('Backend service unavailable')));
    ...
    it('/v1/soulchain/persist (POST) - should persist ledger to IPFS', () => {
      return request(app.getHttpServer())
        ...
        .expect((res) => {
          expect(res.body.success).toBe(true);
          expect(res.body).toHaveProperty('cid');
          expect(res.body.message).toBe('Soulchain ledger persisted to IPFS successfully');
        });
    });
  });
    ...
    it('/v1/petals/propose (POST) - should process Petals proposal', () => {
      ...
      return request(app.getHttpServer())
        ...
        .expect((res) => {
          ...
          expect(res.body.message).toBe('Petals proposal processed successfully');
        });
    });
  });
