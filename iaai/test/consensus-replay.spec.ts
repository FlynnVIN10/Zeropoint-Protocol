...
describe('Consensus Replay Tests', () => {
  ...
  describe('Token-Gated Operations', () => {
    it('should pass consensus intent with sufficient stake', async () => {
      ...
      const result = await appController.validateConsensusIntent({ intent, stakeAmount });
    ...
    it('should pass consensus sync with valid data', async () => {
      ...
      const consensusData = {
        votes: [
          ...
          { voter: 'user3', choice: 'no', weight: 80, timestamp: new Date(), signature: 'sig3' }
        ],
        quorum: 3,
        threshold: 0.67,
        timestamp: new Date()
      ...
      const result = await appController.syncConsensusWithDAOState({ proposalId, consensusData });
    ...
    it('should pass consensus with sufficient agreement', async () => {
      ...
      const result = await appController.processConsensusPass({ proposalId, votes });
    ...
    it('should fail consensus with insufficient agreement', async () => {
      ...
      const votes = [
        { voter: 'user1', choice: 'yes', weight: 100, timestamp: new Date(), signature: 'sig1' },
        { voter: 'user2', choice: 'no', weight: 150, timestamp: new Date(), signature: 'sig2' },
        { voter: 'user3', choice: 'no', weight: 80, timestamp: new Date(), signature: 'sig3' }
      ];
