...
import { FinetuneConfig, TrainingMetrics, CheckpointInfo, RecipeTemplate } from '../../src/tinygrad/llm-base-recipes.types';
...
describe('Phase C2: Recipes LLM Base', () => {
  ...
  describe('Milestone C2: Recipes: LLM Base', () => {
    ...
    it('should perform checkpoint integrity checks', async () => {
      ...
      const checkpoint = await llmRecipes.createCheckpoint(recipe, 'test-checkpoint');
    ...
    it('should achieve monotonic loss convergence', async () => {
      ...
      for (let i = 1; i < metrics.lossHistory.length; i++) {
        ...
        if (currentLoss > previousLoss + 0.01) { // Allow 0.01 tolerance
          nonDecreasingCount++;
        }
      ...
      const decreasingRatio = (metrics.lossHistory.length - 1 - nonDecreasingCount) / (metrics.lossHistory.length - 1);
    ...
    it('should achieve ≥10% perplexity reduction or ≥5-point task gain', async () => {
      ...
      if (metrics.perplexity && metrics.perplexity.baseline && metrics.perplexity.final) {
        const perplexityReduction = (metrics.perplexity.baseline - metrics.perplexity.final) / metrics.perplexity.baseline;
        expect(perplexityReduction).toBeGreaterThan(0.1); // ≥10% reduction
      }
      
      if (metrics.taskAccuracy && metrics.taskAccuracy.baseline && metrics.taskAccuracy.final) {
        const taskGain = metrics.taskAccuracy.final - metrics.taskAccuracy.baseline;
    ...
    it('should produce equivalent metrics with same seed', async () => {
      ...
      if (metrics1.lossHistory && metrics2.lossHistory) {
        const finalLoss1 = metrics1.lossHistory[metrics1.lossHistory.length - 1];
        const finalLoss2 = metrics2.lossHistory[metrics2.lossHistory.length - 1];
    ...
    it('should support different model architectures (8B, 13B)', async () => {
      ...
      for (const size of modelSizes) {
        ...
        expect(Math.abs(actualParams - expectedParams) / expectedParams).toBeLessThan(paramTolerance);
      }
    });
    ...
    it('should handle training interruptions gracefully', async () => {
      ...
      const resumeResult = await llmRecipes.resumeTraining(trainingRun.id, lastCheckpoint.path);
