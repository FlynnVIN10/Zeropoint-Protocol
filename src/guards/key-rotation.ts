...
export class KeyRotationGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    try {
      ...
      if (payload.keyId && payload.keyId !== await this.keyRotationService.getCurrentKeyId()) {
        // Token is using old key, but still valid - log for rotation
        await this.keyRotationService.logKeyRotation(payload.keyId, 'deprecated_key_used');
      ...
      await this.keyRotationService.logFailedAuthentication(token, request.ip, error.message);
