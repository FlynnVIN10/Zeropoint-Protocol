...
export const onRequest: PagesFunction<Env> = async (ctx) => {
	try {
		...
		const upstream = env.TINYGRAD_EXEC_UPSTREAM;
		...
		if (upstream && prompt) {
			const r = await fetch(upstream, {
				...
				headers: {
					...
					...(env.TINYGRAD_TOKEN ? { authorization: `Bearer ${env.TINYGRAD_TOKEN}` } : {})
				},
				body: JSON.stringify({ q: prompt })
			});
			if (r.ok) return new Response(r.body, { headers: jsonHeaders() });
		}
		return new Response(JSON.stringify({ response: 'Tinygrad proxy active: upstream not configured. This is a implementation response.' }), { headers: jsonHeaders() });
	} catch (e: any) {
