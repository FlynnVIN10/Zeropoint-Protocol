...
describe('Scope Enforcement Framework', () => {
  ...
  describe('Scope Boundary Validation', () => {
    it('should allow components within allowed scope', () => {
      ...
      expect(service.validateScopeBoundary('wondercraft', 'simulation')).toBe(true);
      expect(service.validateScopeBoundary('website-v2', 'deployment')).toBe(true);
    });

    it('should deny components outside allowed scope', () => {
      expect(service.validateScopeBoundary('external-service', 'api-call')).toBe(false);
      expect(service.validateScopeBoundary('third-party-lib', 'import')).toBe(false);
      expect(service.validateScopeBoundary('unrelated-component', 'operation')).toBe(false);
    });
    ...
    it('should deny explicitly denied components', () => {
      ...
      expect(service.validateScopeBoundary('synthetic-data', 'generation')).toBe(false);
    });
  ...
  describe('Mock Implementation Detection', () => {
    it('should detect mock patterns in code', () => {
      expect(service.detectMockImplementation('const mockService = new MockService()', 'test.ts')).toBe(true);
      expect(service.detectMockImplementation('const fakeData = generateFakeData()', 'data.ts')).toBe(true);
      expect(service.detectMockImplementation('const syntheticResult = createSynthetic()', 'result.ts')).toBe(true);
    });

    it('should not detect non-mock code', () => {
      expect(service.detectMockImplementation('const realService = new RealService()', 'service.ts')).toBe(false);
      expect(service.detectMockImplementation('const actualData = fetchRealData()', 'data.ts')).toBe(false);
      expect(service.detectMockImplementation('const genuineResult = processRealData()', 'result.ts')).toBe(false);
    });
    ...
    it('should record violations for detected mocks', () => {
      service.detectMockImplementation('const mockService = new MockService()', 'test.ts');
  ...
  describe('Synthetic Data Validation', () => {
    it('should detect synthetic data sources', () => {
      expect(service.validateSyntheticData('mock-dataset', 'training-data')).toBe(false);
      expect(service.validateSyntheticData('fake-images', 'vision-data')).toBe(false);
      expect(service.validateSyntheticData('synthetic-text', 'nlp-data')).toBe(false);
    });

    it('should allow real data sources', () => {
      expect(service.validateSyntheticData('real-dataset', 'training-data')).toBe(true);
      expect(service.validateSyntheticData('actual-images', 'vision-data')).toBe(true);
      expect(service.validateSyntheticData('genuine-text', 'nlp-data')).toBe(true);
    });
  ...
  describe('Hardware Interaction Validation', () => {
    it('should detect simulated hardware interactions', () => {
      expect(service.validateHardwareInteraction('mock-gpu', 'training-engine')).toBe(false);
      expect(service.validateHardwareInteraction('fake-power', 'monitoring')).toBe(false);
      expect(service.validateHardwareInteraction('synthetic-temperature', 'sensor')).toBe(false);
    });

    it('should allow real hardware interactions', () => {
      expect(service.validateHardwareInteraction('real-gpu', 'training-engine')).toBe(true);
      expect(service.validateHardwareInteraction('actual-power', 'monitoring')).toBe(true);
      expect(service.validateHardwareInteraction('genuine-temperature', 'sensor')).toBe(true);
    });
  ...
  describe('Compliance Reporting', () => {
    ...
    it('should not allow negative compliance scores', () => {
      ...
      for (let i = 0; i < 10; i++) {
        service.detectMockImplementation(`const fakeData${i} = {}`, `data${i}.ts`);
      }
  ...
  describe('Utility Functions', () => {
    it('should provide validateScope utility function', () => {
      ...
      const localService = new ScopeEnforcementService({ failOnViolation: false });
      expect(localService.validateScopeBoundary('tinygrad', 'training')).toBe(true);
      expect(localService.validateScopeBoundary('mock-service', 'test')).toBe(false);
    });

    it('should provide detectMock utility function', () => {
      // Test with local service instances to avoid global state issues
      const localService = new ScopeEnforcementService({ failOnViolation: false });
      expect(localService.detectMockImplementation('const realService = {}', 'service.ts')).toBe(false);
      expect(localService.detectMockImplementation('const mockService = {}', 'service.ts')).toBe(true);
    });

    it('should provide validateData utility function', () => {
      // Test with local service instances to avoid global state issues
      const localService = new ScopeEnforcementService({ failOnViolation: false });
      expect(localService.validateSyntheticData('real-dataset', 'training-data')).toBe(true);
      expect(localService.validateSyntheticData('mock-dataset', 'training-data')).toBe(false);
    });

    it('should provide validateHardware utility function', () => {
      // Test with local service instances to avoid global state issues
      const localService = new ScopeEnforcementService({ failOnViolation: false });
      expect(localService.validateHardwareInteraction('real-gpu', 'training')).toBe(true);
      expect(localService.validateHardwareInteraction('mock-gpu', 'training')).toBe(false);
    });
  });

  describe('Error Handling', () => {
    it('should throw error on critical violations when failOnViolation is true', () => {
      const failingService = new ScopeEnforcementService({ failOnViolation: true });
      expect(() => {
        failingService.detectMockImplementation('const fakeData = {}', 'data.ts');
      }).toThrow('Scope violation: Mock implementation detected in code at data.ts');
    });

    it('should not throw error when failOnViolation is false', () => {
      const nonFailingService = new ScopeEnforcementService({ failOnViolation: false });
      
      expect(() => {
        nonFailingService.detectMockImplementation('const fakeData = {}', 'data.ts');
      }).not.toThrow();
    });
