type Env = {
	WONDERCRAFT_STATUS_UPSTREAM?: string;
};
...
export const onRequest: PagesFunction<Env> = async (ctx) => {
	try {
		const upstream = ctx.env.WONDERCRAFT_STATUS_UPSTREAM;
		if (upstream) {
			const r = await fetch(upstream, { headers: { accept: 'application/json' } });
			if (r.ok) return new Response(r.body, { headers: jsonHeaders() });
		}
		const localUrl = new URL('/wondercraft/status.json', ctx.request.url).toString();
