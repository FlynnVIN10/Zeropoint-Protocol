...
describe("NestJS Gateway API (Phase 2)", () => {
  ...
  describe("Python Backend Integration", () => {
    describe("generateText", () => {
      ...
      it("should handle Python backend errors", async () => {
        ...
        await expect(appController.generateText(dto)).rejects.toThrow(
          "Backend error",
        );
      });
    });
  ...
  describe("Authentication System", () => {
    describe("register", () => {
      ...
      it("should reject duplicate username", async () => {
        ...
        const dto = { username: "testuser", password: "password123" };
        await expect(appController.register(dto)).rejects.toThrow(
          HttpException,
        );
      });
  ...
  describe("IPFS Integration", () => {
    describe("uploadFile", () => {
      ...
      it("should handle missing file", async () => {
        await expect(
          appController.uploadFile(null, "Test rationale"),
      ...
      it("should handle upload errors", async () => {
        ...
        await expect(
          appController.uploadFile(mockFile, "Test rationale"),
        ).rejects.toThrow(HttpException);
      });
    });
    ...
    describe("downloadFile", () => {
      it("should download file from IPFS successfully", async () => {
        ...
        await appController.downloadFile(
          "test-cid",
          "Test download rationale",
          mockRes,
        );
        ...
        expect(mockRes.set).toHaveBeenCalledWith(
          "Content-Disposition",
          'attachment; filename="file-test-cid"',
        );
    ...
    describe("listDirectory", () => {
      ...
      it("should handle listing errors", async () => {
        jest
          .spyOn(appService, "listDirectory")
          .mockRejectedValue(new Error("Listing failed"));

        await expect(appController.listDirectory("test-cid")).rejects.toThrow(
          HttpException,
        );
      });
    });
  ...
  describe("Soulchain Integration", () => {
    describe("persistSoulchain", () => {
      it("should persist soulchain ledger to IPFS", async () => {
        const {
          soulchain,
        } = require("../agents/soulchain/soulchain.ledger.js");
      ...
      it("should handle persistence errors", async () => {
        ...
        soulchain.persistLedgerToIPFS.mockRejectedValue(
          new Error("Persistence failed"),
        );

        await expect(appController.persistSoulchain()).rejects.toThrow(
          HttpException,
        );
      });
    });
  ...
  describe("Petals Integration", () => {
    describe("proposeWithPetals", () => {
      ...
      it("should handle Petals proposal errors", async () => {
        ...
        jest
          .spyOn(appService, "proposeWithPetals")
          .mockRejectedValue(new Error("Petals error"));

        await expect(
          appController.proposeWithPetals(mockProposal),
        ).rejects.toThrow(HttpException);
      });
    });
  });
  ...
  describe("Error Handling", () => {
    it("should handle Zeroth-gate violations", async () => {
      ...
      await expect(appController.getHello()).rejects.toThrow(
        "Zeroth violation: getHello blocked.",
      );
    });
  ...
  describe("Validation", () => {
    ...
    it("should handle missing required fields", async () => {
      ...
      jest
        .spyOn(appService, "generateText")
        .mockResolvedValue({ generated_text: "test" });
