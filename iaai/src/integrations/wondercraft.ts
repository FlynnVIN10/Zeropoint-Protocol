...
export class WondercraftIntegration extends EventEmitter {
  ...
  constructor(config?: Partial<WondercraftConfig>) {
    ...
    this.config = {
      ...
      webhookSecret:
        process.env.WONDERCRAFT_WEBHOOK_SECRET ||
  ...
  async listBriefings(filters?: {
    ...
    if (filters?.category) {
      briefings = briefings.filter(
        (b) => b.metadata.category === filters.category,
      );
    }

    if (filters?.speaker) {
      briefings = briefings.filter(
        (b) => b.metadata.speaker === filters.speaker,
      );
    }
  ...
  private validateAudioUrl(url: string): void {
    ...
    if (!this.config.supportedFormats.includes(format)) {
      throw new Error(
        `Unsupported audio format: ${format}. Supported formats: ${this.config.supportedFormats.join(", ")}`,
      );
    }
  }
  ...
  private validateOptInRequest(request: OptInRequest): void {
    ...
    if (
      ...
      typeof request.consentTimestamp !== "number"
    ) {
