...
import { synthiantRegistry, SynthiantAgent, CurrentResourceUsage } from './registry';
...
export class SynthiantRuntime extends EventEmitter {
  ...
  async executeTask(
    ...
    try {
      ...
      const monitorPromise = this.resourceMonitor.startMonitoring(taskId, agentId);
      
      // Execute task with timeout
      const taskPromise = this.executeWithTimeout(taskFn, this.config.maxExecutionTime);
      ...
      this.emit('task_failed', { taskId, agentId, error: error.message, result });
      throw error;
  ...
  async terminateAgentTasks(agentId: string): Promise<number> {
    ...
    for (const [taskId, context] of this.activeTasks.entries()) {
      if (context.agentId === agentId) {
        ...
        // Log termination
        this.logExecutionStep(context, 'task_terminated', 'execution', 0, 'failure', {
        ...
        this.emit('task_terminated', { taskId, agentId, reason: 'agent_terminated' });
      }
    }
  ...
  private setupEventHandlers(): void {
    this.on('task_started', (data) => {
      console.log(`[Runtime] Task started: ${data.taskId} for agent ${data.agentId}`);
    });

    this.on('task_completed', (data) => {
      console.log(`[Runtime] Task completed: ${data.taskId} in ${data.executionTime.toFixed(2)}ms`);
    });
    ...
    this.on('execution_step', (data) => {
      console.debug(`[Runtime] Execution step: ${data.step.action} on ${data.step.resource}`);
    });
  }
  ...
  getRuntimeStats(): {
    ...
    const totalMemory = activeTasks.reduce((sum, task) => sum + task.resourceUsage.memory, 0);
    const totalCPU = activeTasks.reduce((sum, task) => sum + task.resourceUsage.cpu, 0);

    return {
      activeTasks: this.activeTasks.size,
      totalMemory,
      totalCPU,
      uptime: Date.now() - (this.activeTasks.size > 0 ? 
        Math.min(...activeTasks.map(t => t.startTime)) : Date.now())
    };
  }
}
...
class ResourceMonitor {
  ...
  private updateResourceUsage(taskId: string, agentId: string): void {
    ...
    usage.memory = Math.min(usage.memory + Math.random() * 10, this.config.maxMemoryUsage);
    
    // Update CPU usage (simulated)
    usage.cpu = Math.min(usage.cpu + Math.random() * 0.1, this.config.maxCPUUsage);
    
    // Check quotas
    if (this.config.enableQuotaEnforcement) {
      if (usage.memory > this.config.maxMemoryUsage) {
        this.handleQuotaBreach(taskId, 'memory', usage.memory, this.config.maxMemoryUsage);
      }
      
      if (usage.cpu > this.config.maxCPUUsage) {
        this.handleQuotaBreach(taskId, 'cpu', usage.cpu, this.config.maxCPUUsage);
      }
      
      if (usage.time > this.config.maxExecutionTime / 1000) {
        this.handleQuotaBreach(taskId, 'time', usage.time, this.config.maxExecutionTime / 1000);
      }
    }
  }
  ...
  private handleQuotaBreach(
    ...
    console.warn(`Quota breach detected for task ${taskId}: ${resource} ${current}/${limit}`);
   ...
   */
  updateSpecificUsage(taskId: string, updates: Partial<CurrentResourceUsage>): void {
    const usage = this.resourceUsage.get(taskId);
    if (usage) {
...
export const synthiantRuntime = new SynthiantRuntime(defaultRuntimeConfig, defaultSandboxPolicy);
