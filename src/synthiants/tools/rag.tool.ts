...
export class RAGTool implements ToolInterface {
  ...
  public readonly description =
    "Retrieval-Augmented Generation tool for knowledge base querying";
  ...
  private async performQuery(params: RAGQuery): Promise<RAGResult[]> {
    ...
    const threshold =
      params.similarityThreshold || this.config.defaultSimilarityThreshold;
  ...
  private checkRateLimit(): void {
    ...
    if (this.queryCount >= this.config.rateLimitPerHour) {
      throw new Error(
        `Rate limit exceeded: ${this.config.rateLimitPerHour} queries per hour`,
      );
    }
  }
