...
export class AgentStateController {
  ...
  async createAgentState(
    @Body(ValidationPipe) dto: CreateAgentStateDto,
  ): Promise<AgentState> {
  ...
  async getAgentsByEthicalRating(
    @Param("rating") rating: "aligned" | "warn" | "reject",
  ): Promise<AgentState[]> {
  ...
  async recordError(
    ...
    return this.agentStateService.recordError(
      agentId,
      body.error,
      body.context,
    );
  ...
  async addReflection(
    ...
    return this.agentStateService.addReflection(
      agentId,
      body.content,
      body.tags || [],
    );
