...
async function runAgentLifecycle(agentIds: string[]) {
  ...
  for (const agentId of agentIds) {
    // 1. Introspection
    const reflection = await introspect.ask(
      agentId,
    ...
    let tags = [];
    if (
      reflection.summaryData &&
      reflection.summaryData.timeline &&
      reflection.summaryData.timeline[0] &&
      reflection.summaryData.timeline[0].tags
    ) {
    ...
    if (flaggedFunctions.length > 0) {
      const proposal = await train.proposeRewrite(
        agentMeta,
        flaggedFunctions[0],
      );
    ...
    console.log(
      `Agent ${agentId} completed lifecycle with ${perf.metrics.runtime}ms runtime`,
    );
  }
}
