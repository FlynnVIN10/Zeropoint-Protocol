...
describe("Security Unit Tests", () => {
  ...
  beforeAll(async () => {
    ...
    securityInterceptor = module.get<SecurityLoggingInterceptor>(
      SecurityLoggingInterceptor,
    );
  ...
  describe("Security Middleware", () => {
    ...
    it("should apply correct rate limits for different endpoints", () => {
      expect(securityMiddleware["getRateLimitForEndpoint"]("/auth/login")).toBe(
        5,
      );
      expect(
        securityMiddleware["getRateLimitForEndpoint"]("/v1/advanced/status"),
      ).toBe(10);
      expect(
        securityMiddleware["getRateLimitForEndpoint"]("/v1/agent/state"),
      ).toBe(20);
      expect(securityMiddleware["getRateLimitForEndpoint"]("/v1/health")).toBe(
        100,
      );
    });
    ...
    it("should extract client IP correctly", () => {
      ...
      expect(securityMiddleware["getClientIP"](mockRequest)).toBe(
        "192.168.1.1",
      );
  ...
  describe("Security Logging Interceptor", () => {
    it("should determine action correctly", () => {
      ...
      expect(securityInterceptor["determineAction"](mockRequest)).toBe(
        "AUTH_ATTEMPT",
      );
  ...
  describe("Rate Limiting Logic", () => {
    it("should track rate limiting correctly", () => {
      ...
      const isRateLimited = securityMiddleware["isRateLimited"](
        testIP,
        testURL,
      );
