...
export class LLMBaseRecipes {
  ...
  async getRecipeTemplates(): Promise<RecipeTemplate[]> {
    const templates: RecipeTemplate[] = [
      ...
      },
      {
        ...
        description: '13B parameter model finetuning for advanced conversational AI',
  ...
  async createRecipe(config: FinetuneConfig): Promise<RecipeTemplate & { 
  ...
  async getTrainingMetrics(runId: string): Promise<TrainingMetrics> {
    ...
    if (!metrics) {
      ...
      if (metrics.perplexity.history.length < currentStep) {
        const lastPerp = metrics.perplexity.history[metrics.perplexity.history.length - 1];
      ...
      if (metrics.taskAccuracy.history.length < currentStep) {
        const lastAcc = metrics.taskAccuracy.history[metrics.taskAccuracy.history.length - 1];
  ...
  async createCheckpoint(recipe: RecipeTemplate, name: string): Promise<CheckpointInfo> {
  ...
  async getTrainingStatus(runId: string): Promise<TrainingStatus> {
    ...
    return {
      ...
      lastCheckpoint: checkpoints.length > 0 ? checkpoints[checkpoints.length - 1] : undefined
    };
  }
  ...
  async resumeTraining(runId: string, checkpointPath: string): Promise<ResumeResult> {
    const trainingRun = this.trainingRuns.get(runId);
    if (!trainingRun) {
      return {
        success: false,
