...
export class OAuthController {
  ...
  async authorize(
    ...
    try {
      this.logger.log(`OAuth authorization request: ${clientId} -> ${redirectUri}`);
  ...
  async token(
    ...
    try {
      this.logger.log(`OAuth token request: ${grantType} for client ${clientId}`);

      if (grantType === 'authorization_code') {
        const token = await this.oauthService.authenticateWithOAuth('mock', code, redirectUri);
