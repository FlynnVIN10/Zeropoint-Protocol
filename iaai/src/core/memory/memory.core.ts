...
import {
  ...
  MemorySummary,
  createMemoryEntry,
} from "./memory.types.js"; // Schema import
...
export class MemoryCore {
  async recordExperience(agentId: string, event: MemoryEntry): Promise<void> {
    ...
    if (event.ethicalRating === "reject") {
      console.warn(
        `Memory entry for ${agentId} flagged: Ethical rejection logged.`,
      );
    }
  }
  ...
  summarizeHistory(agentId: string): MemorySummary {
    ...
    entries.forEach(
      (e) =>
        (domainCounts[e.context.domain] =
          (domainCounts[e.context.domain] || 0) + 1),
    );
    ...
    entries.forEach((e) => {
      const intentTag = e.tags.find((t) => t.type === "#intent") as {
        purpose: string;
      };
      if (intentTag)
        intentCounts[intentTag.purpose] =
          (intentCounts[intentTag.purpose] || 0) + 1;
    });
    const dominantIntent =
      Object.entries(intentCounts).sort((a, b) => b[1] - a[1])[0]?.[0] || "";

    const ethicsRatio = entries.reduce(
      (ratio, e) => {
