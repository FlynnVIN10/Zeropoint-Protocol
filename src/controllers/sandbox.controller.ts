...
export class SandboxController {
  ...
  async executeCommand(
    ...
    const result = await this.sandboxService.executeInSandbox(sandboxId, body.command);
    return {
      status: 'success',
      data: result,
      timestamp: new Date().toISOString(),
  ...
  async getSandboxLogs(
    ...
    const logs = await this.sandboxService.getSandboxLogs(sandboxId, body.lines || 100);
    return {
      status: 'success',
      data: {
        sandboxId,
  ...
  streamSandboxStatus(@Param('sandboxId') sandboxId: string): Observable<MessageEvent> {
