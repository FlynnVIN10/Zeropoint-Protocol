...
describe("JWT Authentication System", () => {
  ...
  describe("JwtStrategy", () => {
    ...
    it("should throw UnauthorizedException for invalid payload structure", async () => {
      ...
      await expect(
        jwtStrategy.validate(invalidPayload as JwtPayload),
      ).rejects.toThrow(
  ...
  describe("JwtAuthGuard", () => {
    ...
    it("should block authentication on Zeroth violation", async () => {
      ...
      await expect(
        jwtAuthGuard.canActivate(mockExecutionContext as any),
      ).rejects.toThrow(
    ...
    it("should reject requests without Authorization header", async () => {
      ...
      await expect(
        jwtAuthGuard.canActivate(contextWithoutAuth as any),
      ).rejects.toThrow(
    ...
    it("should reject requests with invalid Authorization header format", async () => {
      ...
      await expect(
        jwtAuthGuard.canActivate(contextWithInvalidAuth as any),
      ).rejects.toThrow(
        new UnauthorizedException("Missing or invalid Authorization header"),
      );
    });
    ...
    it("should handle guard errors gracefully", async () => {
      ...
      await expect(
        jwtAuthGuard.canActivate(invalidContext as any),
      ).rejects.toThrow();
      expect(mockSoulchain.addXPTransaction).toHaveBeenCalledWith(
        expect.objectContaining({
          agentId: "anonymous",
  ...
  describe("AuthService", () => {
    ...
    it("should block registration on Zeroth violation", async () => {
      ...
      await expect(
        authService.register(registerDto, "127.0.0.1", "test-agent"),
      ).rejects.toThrow(
    ...
    it("should reject registration for existing user", async () => {
      ...
      await expect(
        authService.register(registerDto, "127.0.0.1", "test-agent"),
      ).rejects.toThrow(new Error("Username or email already exists"));
      expect(mockSoulchain.addXPTransaction).toHaveBeenCalledWith(
        expect.objectContaining({
          agentId: "newuser",
          amount: -10,
