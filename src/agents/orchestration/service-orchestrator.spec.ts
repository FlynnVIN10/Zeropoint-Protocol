...
import { ServiceOrchestrator, OrchestrationRequest, OperationRequest, OrchestrationResponse } from './service-orchestrator.js';
...
describe('ServiceOrchestrator', () => {
  ...
  describe('constructor', () => {
    it('should initialize service health tracking', () => {
      ...
      expect(health.map(h => h.service)).toEqual(['petals', 'ai-generation', 'validation', 'analysis']);
  ...
  describe('orchestrateServices', () => {
    it('should successfully orchestrate services with valid request', async () => {
      const result = await orchestrator.orchestrateServices(sampleOrchestrationRequest);
    ...
    it('should throw error when Zeroth-gate validation fails', async () => {
      ...
      await expect(orchestrator.orchestrateServices(sampleOrchestrationRequest))
        .rejects.toThrow('Zeroth violation: Orchestration request blocked by validation.');
    });
    ...
    it('should throw error when operation validation fails', async () => {
      ...
      await expect(orchestrator.orchestrateServices(invalidRequest))
        .rejects.toThrow('Zeroth violation: Orchestration request blocked by validation.');
    });
    ...
    it('should handle orchestration with multiple operations', async () => {
      ...
      const result = await orchestrator.orchestrateServices(multiOperationRequest);
    ...
    it('should log successful orchestration to Soulchain', async () => {
      ...
      expect(mockSoulchain.addXPTransaction).toHaveBeenCalledWith({
        ...
        rationale: expect.stringContaining('Orchestration successful: 1/1 operations'),
        timestamp: expect.any(String),
        previousCid: null,
        tags: expect.arrayContaining([
          expect.objectContaining({ type: '#intent', purpose: '#service-orchestration' }),
          expect.objectContaining({ type: '#thread', taskId: 'test-orchestration-id' })
        ])
      });
    });
    ...
    it('should log orchestration error to Soulchain when orchestration fails', async () => {
      ...
      expect(mockSoulchain.addXPTransaction).toHaveBeenCalledWith({
        ...
        previousCid: null,
        tags: expect.arrayContaining([
          expect.objectContaining({ type: '#intent', purpose: '#orchestration-error' })
        ])
      });
    });
  });
  ...
  describe('private validation methods', () => {
    describe('validatePetalsOperation', () => {
      it('should return true for valid Petals operation', async () => {
        const result = await (orchestrator as any).validatePetalsOperation(sampleOperation);
      ...
      it('should return false for malicious code', async () => {
        ...
        const result = await (orchestrator as any).validatePetalsOperation(maliciousOperation);
      ...
      it('should return false for resource abuse', async () => {
        const abusiveOperation = {
          ...sampleOperation,
          data: { agentId: 'test-agent', code: 'while(true) { /* infinite loop */ }' }
        };
        const result = await (orchestrator as any).validatePetalsOperation(abusiveOperation);
        expect(result).toBe(false);
      });
      ...
      it('should return false for privacy violations', async () => {
        ...
        const result = await (orchestrator as any).validatePetalsOperation(privacyOperation);
        expect(result).toBe(false);
      });
    });
    ...
    describe('validateAIGenerationOperation', () => {
      it('should return true for valid AI generation operation', async () => {
        ...
        const result = await (orchestrator as any).validateAIGenerationOperation(aiOperation);
        expect(result).toBe(true);
      });
    ...
    describe('validateValidationOperation', () => {
      it('should return true for valid validation operation', async () => {
        ...
        const result = await (orchestrator as any).validateValidationOperation(validationOperation);
        expect(result).toBe(true);
      });
    ...
    describe('validateAnalysisOperation', () => {
      it('should return true for valid analysis operation', async () => {
        ...
        const result = await (orchestrator as any).validateAnalysisOperation(analysisOperation);
        expect(result).toBe(true);
      });
    ...
    describe('checkForMaliciousPatterns', () => {
      it('should return true for safe code', async () => {
        ...
        const result = await (orchestrator as any).checkForMaliciousPatterns(safeCode);
        expect(result).toBe(true);
      });

      it('should return false for eval()', async () => {
        const maliciousCode = 'eval("malicious code");';
        const result = await (orchestrator as any).checkForMaliciousPatterns(maliciousCode);
        expect(result).toBe(false);
      });
    ...
    describe('checkForResourceAbuse', () => {
      it('should return true for safe code', async () => {
        ...
        const result = await (orchestrator as any).checkForResourceAbuse(safeCode);
        expect(result).toBe(true);
      });

      it('should return false for infinite while loop', async () => {
        const abusiveCode = 'while(true) { /* infinite loop */ }';
        const result = await (orchestrator as any).checkForResourceAbuse(abusiveCode);
        expect(result).toBe(false);
      });
    ...
    describe('checkForPrivacyViolations', () => {
      it('should return true for safe code', async () => {
        ...
        const result = await (orchestrator as any).checkForPrivacyViolations(safeCode);
        expect(result).toBe(true);
      });

      it('should return false for password assignment', async () => {
        const privacyCode = 'password = "secret123";';
        const result = await (orchestrator as any).checkForPrivacyViolations(privacyCode);
        expect(result).toBe(false);
      });
  ...
  describe('groupOperationsByDependencies', () => {
    it('should group operations without dependencies correctly', () => {
      ...
      const groups = (orchestrator as any).groupOperationsByDependencies(operations);
  ...
  describe('executeSingleOperation', () => {
    it('should execute Petals operation correctly', async () => {
      const result = await (orchestrator as any).executeSingleOperation(sampleOperation, []);
      expect(result).toBeDefined();
      expect(result.rewrittenCode).toBe('function test() { return "hello world"; }');
    ...
    it('should execute AI generation operation correctly', async () => {
      ...
      const result = await (orchestrator as any).executeSingleOperation(aiOperation, []);
      expect(result).toBeDefined();
      expect(result.generatedText).toContain('Generated content for: Generate text');
    ...
    it('should execute validation operation correctly', async () => {
      ...
      const result = await (orchestrator as any).executeSingleOperation(validationOperation, []);
      expect(result).toBeDefined();
    ...
    it('should execute analysis operation correctly', async () => {
      ...
      const result = await (orchestrator as any).executeSingleOperation(analysisOperation, []);
      expect(result).toBeDefined();
    ...
    it('should throw error for unknown operation type', async () => {
      ...
      await expect((orchestrator as any).executeSingleOperation(unknownOperation, []))
  ...
  describe('calculateOrchestrationSummary', () => {
    it('should calculate correct summary for successful operations', () => {
      ...
      const summary = (orchestrator as any).calculateOrchestrationSummary(results, processingTime);
  ...
  describe('logging methods', () => {
    it('should log violation correctly', async () => {
      await (orchestrator as any).logViolation(sampleOrchestrationRequest, 'test reason');
      ...
      expect(mockSoulchain.addXPTransaction).toHaveBeenCalledWith({
        ...
        previousCid: null,
        tags: expect.arrayContaining([
          expect.objectContaining({ type: '#intent', purpose: '#security-violation' })
        ])
      });
    });
    ...
    it('should log successful orchestration correctly', async () => {
      ...
      await (orchestrator as any).logSuccessfulOrchestration(sampleOrchestrationRequest, results, summary);
    ...
    it('should log orchestration error correctly', async () => {
      ...
      await (orchestrator as any).logOrchestrationError(sampleOrchestrationRequest, error, 150);
