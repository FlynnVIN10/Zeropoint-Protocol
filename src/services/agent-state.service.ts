...
import {
  ...
  AgentContext,
  AgentMemory,
} from "../entities/agent-state.entity.js";
...
export class AgentStateService {
  ...
  async createAgentState(dto: CreateAgentStateDto): Promise<AgentState> {
    ...
    if (!checkIntent(dto.agentId + dto.name + dto.did)) {
      throw new BadRequestException(
        "Zeroth violation: Agent creation blocked due to ethical concerns.",
      );
    }
    ...
    if (existingAgent) {
      throw new BadRequestException(
        `Agent with ID ${dto.agentId} already exists.`,
      );
    }
    ...
    // Log to Soulchain
    await this.logToSoulchain(
      savedAgent,
      "agent_created",
      10,
      "New agent state created",
    );
  ...
  async updateAgentState(
    agentId: string,
    dto: UpdateAgentStateDto,
  ): Promise<AgentState> {
    // Zeroth-gate validation
    if (!checkIntent(agentId + JSON.stringify(dto))) {
      throw new BadRequestException(
        "Zeroth violation: Agent state update blocked due to ethical concerns.",
      );
    }
  ...
  async deleteAgentState(agentId: string): Promise<void> {
    ...
    if (!checkIntent(agentId + "delete")) {
      throw new BadRequestException(
        "Zeroth violation: Agent deletion blocked due to ethical concerns.",
      );
    }
  ...
  async queryAgentStates(
    ...
    const [agents, total] =
      await this.agentStateRepository.findAndCount(options);
    ...
    if (query.trustLevel) {
      filteredAgents = filteredAgents.filter(
        (agent) => agent.getTrustLevel() === query.trustLevel,
      );
    }

    if (query.performanceStatus) {
      filteredAgents = filteredAgents.filter(
        (agent) => agent.getPerformanceStatus() === query.performanceStatus,
      );
    }
   ...
   */
  async getAgentsByEthicalRating(
    rating: "aligned" | "warn" | "reject",
  ): Promise<AgentState[]> {
  ...
  async getAgentsRequiringAttention(): Promise<AgentState[]> {
    ...
    return agents.filter(
      (agent) =>
        agent.metrics.ethicalRating === "warn" ||
        agent.metrics.ethicalRating === "reject",
    );
  }
  ...
  async getAgentStatistics(): Promise<{
    ...
    const stats = {
      ...
      ethicalBreakdown: {
        aligned: agents.filter((a) => a.metrics.ethicalRating === "aligned")
          .length,
        warn: agents.filter((a) => a.metrics.ethicalRating === "warn").length,
        reject: agents.filter((a) => a.metrics.ethicalRating === "reject")
          .length,
      },
      trustBreakdown: {
      ...
      performanceBreakdown: {
        excellent: agents.filter(
          (a) => a.getPerformanceStatus() === "excellent",
        ).length,
