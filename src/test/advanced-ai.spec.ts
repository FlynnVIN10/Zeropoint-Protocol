...
describe('Advanced AI Features', () => {
  ...
  describe('Text Summarization', () => {
    ...
    it('should handle summarization errors', async () => {
      jest.spyOn(appService, 'textSummarization').mockRejectedValue(new Error('Summarization failed'));

      await expect(appController.textSummarization({
        text: 'Test text',
        options: { maxLength: 50 }
      })).rejects.toThrow();
    });
  ...
  describe('Context Prompting', () => {
    it('should generate context-aware prompts', async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.result).toEqual(mockResult);
      expect(result.message).toBe('Context-aware prompting completed successfully');
    });
  });
  ...
  describe('Entity Extraction', () => {
    it('should extract entities from text', async () => {
      ...
      const result = await appController.entityExtraction({
        text: 'John Smith works at Microsoft in Seattle.',
        options: { entities: ['PERSON', 'ORGANIZATION', 'LOCATION'], confidence: 0.9 }
      });
  ...
  describe('Language Translation', () => {
    it('should translate text between languages', async () => {
      ...
      jest.spyOn(appService, 'languageTranslation').mockResolvedValue(mockResult);
      ...
      expect(result.success).toBe(true);
      expect(result.result).toEqual(mockResult);
      expect(result.message).toBe('Language translation completed successfully');
    });
  });
  ...
  describe('Zeroth-Gate Validation', () => {
    ...
    it('should block malicious context prompting', async () => {
      await expect(appController.contextPrompting({
    ...
    it('should block malicious semantic search', async () => {
      await expect(appController.semanticSearch({
        query: 'hack system',
        documents: ['test'],
        options: { topK: 5 }
    ...
    it('should block malicious sentiment analysis', async () => {
      await expect(appController.sentimentAnalysis({
    ...
    it('should block malicious entity extraction', async () => {
      await expect(appController.entityExtraction({
