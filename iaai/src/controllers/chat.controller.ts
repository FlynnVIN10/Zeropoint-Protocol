...
export class ChatController {
  ...
  @Public()
  async requestChange(
    @Body() body: { agentId: string; codeDiff: string; description: string },
  ) {
    try {
  ...
  async vote(@Body() body: VoteRequest) {
    try {
      this.logger.log(
        `Received vote from ${body.voterId} (${body.role}) for proposal ${body.proposalId}`,
      );
      ...
      if (body.role === "sentient") {
        ...
        const approvalVotes = proposal.sentientVotes.filter(
          (v) => v.vote,
        ).length;
        const quorumReached =
          proposal.sentientVotes.length >= Math.ceil(totalSentients * 0.67);
          ...
          if (approved) {
            ...
            this.broadcastToClients({
              ...
              data: {
                ...
                approvalRate: approvalVotes / proposal.sentientVotes.length,
              },
            });
          } else {
  ...
  private async processMessage(
    ...
    if (
      lowerMessage.includes("zeropoint") ||
    ...
    const contextResponse = this.generateContextualResponse(
      lowerMessage,
      recentMessages,
    );
    if (contextResponse) {
  ...
  private generateContextualResponse(
    ...
    const lastAIResponse = recentMessages
    ...
    if (lastAIResponse) {
      ...
      if (
        lastContent.includes("consensus") &&
