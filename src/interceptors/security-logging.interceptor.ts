...
export class SecurityLoggingInterceptor implements NestInterceptor {
  ...
  private logSecurityEvent(event: SecurityEvent): void {
    ...
    if (process.env.NODE_ENV === "development") {
      this.logger.log(
        `Security Event: ${event.action} - ${event.ip} - ${event.statusCode || "pending"}`,
      );
    }
  }
  ...
  getSecurityStats(): any {
    ...
    return {
      ...
      failedRequests: events.filter((e) => e.statusCode && e.statusCode >= 400)
        .length,
      authAttempts: events.filter((e) => e.action === "AUTH_ATTEMPT").length,
      protectedAccess: events.filter((e) => e.action === "PROTECTED_ACCESS")
        .length,
