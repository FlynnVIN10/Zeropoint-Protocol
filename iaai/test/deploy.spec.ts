...
describe("Deployment Script (e2e)", () => {
  ...
  describe("Soulchain Integration", () => {
    it("should have soulchain endpoint available when application is running", async () => {
      try {
        const { stdout } = await execAsync(
          'curl -s -X POST http://localhost:3000/v1/soulchain/persist -H "Content-Type: application/json" -d \'{"action":"test","metadata":{"timestamp":"2025-07-28T00:00:00.000Z"},"data":"test"}\'',
        );
        const response = JSON.parse(stdout);
