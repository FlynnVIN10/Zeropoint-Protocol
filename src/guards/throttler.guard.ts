...
export class CustomThrottlerGuard extends ThrottlerGuard {
  ...
  protected async handleRequest(requestProps: any): Promise<boolean> {
    ...
    try {
      ...
      if (!result) {
        ...
        response.header(
          "Retry-After",
          Math.ceil(requestProps.ttl / 1000).toString(),
        );

        // Log throttling event
        console.warn(
          `Rate limit exceeded for IP: ${ip}, Endpoint: ${endpoint}, Method: ${method}`,
        );
      }
      ...
      console.error(
        `Throttling error for IP: ${ip}, Endpoint: ${endpoint}`,
        error,
      );
      return true; // Allow request on error
    }
  ...
  protected async handleThrottling(
    ...
    // Log throttling event
    console.warn(
      `Request throttled - IP: ${ip}, Endpoint: ${endpoint}, Method: ${method}, Limit: ${limit}, TTL: ${ttl}ms`,
    );
