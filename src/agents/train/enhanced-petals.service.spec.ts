...
import { EnhancedPetalsService, PetalsRequest, PetalsResponse, PetalsBatchRequest, PetalsBatchResponse } from './enhanced-petals.service.js';
...
describe('EnhancedPetalsService', () => {
  ...
  describe('constructor', () => {
    ...
    it('should initialize with custom config values', () => {
      ...
      const customService = new EnhancedPetalsService(httpService, configService);
  ...
  describe('callPetalsAPI', () => {
    ...
    it('should throw error when Zeroth-gate validation fails', async () => {
      ...
      await expect(service.callPetalsAPI(samplePetalsRequest))
        .rejects.toThrow('Zeroth violation: Petals call blocked by enhanced validation.');
    });
    ...
    it('should throw error when code safety validation fails', async () => {
      ...
      await expect(service.callPetalsAPI(maliciousRequest))
        .rejects.toThrow('Zeroth violation: Petals call blocked by enhanced validation.');
    });
    ...
    it('should throw error when resource abuse validation fails', async () => {
      ...
      await expect(service.callPetalsAPI(resourceAbuseRequest))
        .rejects.toThrow('Zeroth violation: Petals call blocked by enhanced validation.');
    });
    ...
    it('should throw error when privacy violation validation fails', async () => {
      ...
      await expect(service.callPetalsAPI(privacyViolationRequest))
        .rejects.toThrow('Zeroth violation: Petals call blocked by enhanced validation.');
    });
    ...
    it('should throw error when tags validation fails', async () => {
      ...
      await expect(service.callPetalsAPI(invalidTagsRequest))
        .rejects.toThrow('Zeroth violation: Petals call blocked by enhanced validation.');
    });
    ...
    it('should log successful operation to Soulchain', async () => {
      ...
      expect(mockSoulchain.addXPTransaction).toHaveBeenCalledWith({
        ...
        tags: expect.arrayContaining([
          ...
          expect.objectContaining({ type: '#intent', purpose: '#code-improvement' }),
          expect.objectContaining({ type: '#thread', taskId: 'petals-operation' }),
  ...
  describe('callPetalsBatch', () => {
    const sampleBatchRequest: PetalsBatchRequest = {
      requests: [samplePetalsRequest, { ...samplePetalsRequest, id: 'test-id-2' }],
    ...
    it('should throw error when all batch requests fail validation', async () => {
      ...
      await expect(service.callPetalsBatch(sampleBatchRequest))
        .rejects.toThrow('Zeroth violation: All batch requests blocked by validation.');
    });
    ...
    it('should log batch operation to Soulchain', async () => {
      ...
      expect(mockSoulchain.addXPTransaction).toHaveBeenCalledWith({
        ...
        tags: expect.arrayContaining([
          ...
          expect.objectContaining({ type: '#intent', purpose: '#batch-processing' }),
  ...
  describe('private validation methods', () => {
    describe('checkForMaliciousPatterns', () => {
      it('should return true for safe code', async () => {
        ...
        const result = await (service as any).checkForMaliciousPatterns(safeCode);
        expect(result).toBe(true);
      });

      it('should return false for eval()', async () => {
        const maliciousCode = 'eval("malicious code");';
        const result = await (service as any).checkForMaliciousPatterns(maliciousCode);
    ...
    describe('checkForResourceAbuse', () => {
      ...
      it('should return false for infinite while loop', async () => {
        ...
        const result = await (service as any).checkForResourceAbuse(abusiveCode);
        expect(result).toBe(false);
      });
    ...
    describe('checkForPrivacyViolations', () => {
      it('should return true for safe code', async () => {
        ...
        const result = await (service as any).checkForPrivacyViolations(safeCode);
        expect(result).toBe(true);
      });

      it('should return false for password assignment', async () => {
        const privacyCode = 'password = "secret123";';
        const result = await (service as any).checkForPrivacyViolations(privacyCode);
        expect(result).toBe(false);
      });
    ...
    describe('checkAgentPermissions', () => {
      it('should return true for any agent (stub implementation)', async () => {
        const result = await (service as any).checkAgentPermissions('any-agent');
        expect(result).toBe(true);
      });
    });
  });
  ...
  describe('calculateBatchSummary', () => {
    it('should calculate correct summary for successful batch', () => {
      ...
      const summary = (service as any).calculateBatchSummary(results, totalRequests);
  ...
  describe('logging methods', () => {
    it('should log violation correctly', async () => {
      await (service as any).logViolation(samplePetalsRequest, 'test-operation', 'test reason');
    ...
    it('should log successful operation correctly', async () => {
      await (service as any).logSuccessfulOperation(samplePetalsRequest, samplePetalsResponse, 100);
    ...
    it('should log error correctly', async () => {
      ...
      expect(mockSoulchain.addXPTransaction).toHaveBeenCalledWith({
        ...
        previousCid: null,
        tags: expect.arrayContaining([
          expect.objectContaining({ type: '#intent', purpose: '#error-handling' })
        ])
      });
    });
    ...
    it('should log batch operation correctly', async () => {
      ...
      await (service as any).logBatchOperation(batchRequest, results, summary, 200);
