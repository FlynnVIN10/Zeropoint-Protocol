...
import {
  ...
  buildThoughtStream,
  mirrorResponse,
} from "./introspect.types.js"; // Schema import
...
export class IntrospectCore {
  ...
  async ask(agentId: string, question: string): Promise<IntrospectiveResponse> {
    ...
    const sourceTags: string[] = summary.timeline.flatMap((entry) =>
      entry.tags.map(
        (tag) =>
          tag.type + ":" + (tag as any).purpose || (tag as any).field || "",
      ),
    );
  ...
  async echo(agentId: string): Promise<string> {
    ...
    const reflection = buildThoughtStream([
      { thought: coreThought, sourceTags: tags.map((t) => t.type) },
    ]);
