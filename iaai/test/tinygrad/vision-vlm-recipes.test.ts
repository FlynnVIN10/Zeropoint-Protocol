...
import {
  ...
  VisionTask,
  SafetyCard,
} from "../../src/tinygrad/vision-vlm-recipes.types";
...
describe("Phase C3: Recipes Vision/VLM", () => {
  ...
  describe("Milestone C3: Recipes: Vision or VLM", () => {
    ...
    it("should commit checkpoints and eval artifacts with hashes", async () => {
      ...
      // Create checkpoint
      const checkpoint = await visionRecipes.createCheckpoint(
        recipe,
        "vision-checkpoint",
      );
      ...
      const integrityCheck = await visionRecipes.verifyCheckpoint(
        checkpoint.path,
      );
    ...
    it("should support multiple vision tasks", async () => {
      ...
      for (const task of tasks) {
        ...
        expect(
          recipe.taskSpecificConfigs[task as VisionTask].lossFunction,
        ).toBeDefined();
        expect(
          recipe.taskSpecificConfigs[task as VisionTask].evaluationMetrics,
        ).toBeDefined();
      }
    });
      ...
      for (const imageSize of imageSizes) {
        for (const patchSize of patchSizes) {
          if (imageSize % patchSize === 0) {
            ...
            expect(recipe.architecture.numPatches).toBe(
              (imageSize / patchSize) ** 2,
            );
          }
        }
      }
    ...
    it("should implement vision-specific loss functions", async () => {
      ...
      const sampleData = {
        images: new Array(32)
          .fill(null)
          .map(() => new Array(224 * 224 * 3).fill(0.5)),
