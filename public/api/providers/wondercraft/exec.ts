type Env = {
	WONDERCRAFT_EXEC_UPSTREAM?: string;
...
export const onRequest: PagesFunction<Env> = async (ctx) => {
	try {
		...
		const upstream = env.WONDERCRAFT_EXEC_UPSTREAM;
		...
		if (upstream && prompt) {
			const r = await fetch(upstream, {
				...
				headers: {
					...
					...(env.WONDERCRAFT_TOKEN ? { authorization: `Bearer ${env.WONDERCRAFT_TOKEN}` } : {})
				},
				body: JSON.stringify({ q: prompt })
			});
			if (r.ok) return new Response(r.body, { headers: jsonHeaders() });
		}
		return new Response(JSON.stringify({ response: 'Wondercraft proxy active: upstream not configured. This is a implementation response.' }), { headers: jsonHeaders() });
	} catch (e: any) {
