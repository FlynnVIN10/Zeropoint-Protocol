...
async function checkEnvironmentFlags(): Promise<HealthCheckResult> {
  ...
  const missingFlags = requiredFlags.filter(flag => process.env[flag] !== '1');
...
async function checkApiReachability(): Promise<HealthCheckResult> {
  try {
    ...
    const { stdout } = await execAsync('curl -s -f http://localhost:3000/health 2>/dev/null || echo "unavailable"');
