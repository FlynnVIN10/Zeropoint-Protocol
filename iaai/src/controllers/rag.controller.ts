...
export class RAGController {
  ...
  async queryContext(@Body() request: RAGQueryRequest): Promise<any> {
    try {
      const response = await this.ragService.queryRAG(
        request.query,
        request.topK || 5,
      );
      ...
      return {
        ...
        data: {
          ...
          sources: request.includeSources
            ? response.sources
    ...
    try {
      ...
      return {
        ...
        data: {
          ...
          message:
            "RAG service is running with enhanced domain-specific capabilities",
        },
        timestamp: new Date().toISOString(),
      };
    } catch (error) {
