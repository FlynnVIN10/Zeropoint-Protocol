  ...
  describe("Custom Validation Decorators", () => {
    ...
    describe("IsValidDID", () => {
      it("should validate DID format", () => {
        ...
        // Valid DIDs
        expect(constraint.validate("did:zeropoint:user123", {} as any)).toBe(
          true,
        );
        expect(
          constraint.validate("did:ethr:0x1234567890abcdef", {} as any),
        ).toBe(true);
        expect(constraint.validate("did:web:example.com", {} as any)).toBe(
          true,
        );
