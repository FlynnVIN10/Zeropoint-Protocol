...
export class StreamController {
        ...
        if (chunk.type === "token" && chunk.content) {
          ...
          res.write(
            `data: ${JSON.stringify({
              ...
              metadata: {
                failoverReason:
                  chunk.metadata?.failoverReason || "provider_failure",
                timestamp: Date.now(),
              },
            })}\n\n`,
          );
  ...
  async getProviderHealth() {
    return {
      status: "success",
      data: {
        providers: Array.from(this.providerHealth.entries()).map(
          ([name, health]) => ({
            name,
  ...
  private async selectProvider(preferredProvider: string): Promise<string> {
    ...
    if (preferredProvider === "auto") {
      ...
      const healthyProviders = availableProviders.filter(
        (p) => this.providerHealth.get(p)?.status === "healthy",
      );
      ...
      return healthyProviders.reduce((best, current) => {
        ...
        return (currentHealth?.failoverCount || 0) <
          (bestHealth?.failoverCount || 0)
          ? current
          : best;
      });
    }
  ...
  private calculateFailoverRate(): number {
    ...
    const totalChecks = providers.reduce(
      (sum, p) => sum + Math.floor((Date.now() - p.lastCheck) / 30000),
      0,
    );
  ...
  private initializeProviderHealth() {
    ...
    this.providerHealth.set("anthropic", {
      status: "healthy",
      lastCheck: Date.now(),
      failoverCount: 0,
    });
  ...
  private async checkProviderHealth() {
    for (const [provider, health] of this.providerHealth.entries()) {
      try {
        ...
        health.lastCheck = Date.now();
        this.logger.error(
          `Provider health check failed for ${provider}:`,
          error,
        );
      }
    }
