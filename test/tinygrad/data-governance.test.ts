...
describe('DataGovernanceService', () => {
  ...
  describe('PII Scanning', () => {
    it('should detect email addresses in content', async () => {
      const piiResult = await governanceService.scanForPII('test-dataset', sampleContent);
      ...
      expect(piiResult.detectedTypes.some(t => t.type === 'email')).toBe(true);
    ...
    it('should detect phone numbers in content', async () => {
      ...
      expect(piiResult.detectedTypes.some(t => t.type === 'phone')).toBe(true);
    ...
    it('should detect credit card numbers in content', async () => {
      ...
      expect(piiResult.detectedTypes.some(t => t.type === 'credit_card')).toBe(true);
      expect(piiResult.samples.some(s => s.type === 'credit_card')).toBe(true);
    });

    it('should detect addresses in content', async () => {
      const piiResult = await governanceService.scanForPII('test-dataset', sampleContent);
      
      expect(piiResult.detectedTypes.some(t => t.type === 'address')).toBe(true);
    ...
    it('should calculate correct PII density', async () => {
      ...
      expect(piiResult.density).toBe(piiResult.totalInstances / sampleContent.length);
    });
    ...
    it('should provide mitigation recommendations', async () => {
      ...
      expect(piiResult.recommendations.every(r => typeof r === 'string')).toBe(true);
    });
    ...
    it('should handle content without PII', async () => {
      ...
      const piiResult = await governanceService.scanForPII('clean-dataset', cleanContent);
    ...
    it('should respect PII detection threshold', async () => {
      ...
      const piiResult = await customService.scanForPII('test-dataset', sampleContent);
      
      // With higher threshold, fewer PII instances should be detected
      expect(piiResult.totalInstances).toBeLessThanOrEqual(
        await governanceService.scanForPII('test-dataset', sampleContent).then(r => r.totalInstances)
      );
    });
  });
  ...
  describe('License Validation', () => {
    ...
    it('should perform multiple compliance checks', async () => {
      ...
      expect(compliance.checks.every(c => 
  ...
  describe('Checksum Generation and Verification', () => {
    it('should generate SHA-256 checksums', async () => {
      const checksums = await governanceService.generateChecksums('test-dataset', sampleContent);
    ...
    it('should verify valid checksums', async () => {
      const checksums = await governanceService.generateChecksums('test-dataset', sampleContent);
      const isValid = await governanceService.verifyChecksums('test-dataset', sampleContent, checksums);
    ...
    it('should detect checksum mismatches', async () => {
      ...
      const isValid = await governanceService.verifyChecksums('test-dataset', modifiedContent, checksums);
    ...
    it('should handle empty content', async () => {
      ...
      const checksums = await governanceService.generateChecksums('empty-dataset', emptyContent);
      
      expect(checksums.sha256).toBeDefined();
      expect(checksums.md5).toBeDefined();
      
      const isValid = await governanceService.verifyChecksums('empty-dataset', emptyContent, checksums);
      expect(isValid).toBe(true);
    });
  });
  ...
  describe('Dataset Manifest Creation', () => {
    ...
    it('should include audit trail in manifest', async () => {
      ...
      expect(manifest.auditTrail.every(entry => 
  ...
  describe('Governance Report Generation', () => {
    it('should generate governance report for date range', async () => {
      ...
      const report = await governanceService.generateGovernanceReport(startDate, endDate);
    ...
    it('should calculate correct dataset summary', async () => {
      ...
      const sum = report.datasetSummary.compliant + 
  ...
  describe('Error Handling', () => {
    ...
    it('should handle empty content arrays', async () => {
      ...
      const piiResult = await governanceService.scanForPII('test-dataset', emptyContent);
      expect(piiResult.totalInstances).toBe(0);
      expect(piiResult.riskLevel).toBe('none');
    });
    ...
    it('should handle null/undefined license information', async () => {
      ...
      const compliance = await governanceService.validateLicense(invalidLicense);
  ...
  describe('Performance and Scalability', () => {
    it('should handle large content arrays efficiently', async () => {
      const largeContent = Array.from({ length: 10000 }, (_, i) => 
        `Line ${i}: This is test content without PII.`
      );
      
      const startTime = Date.now();
      const piiResult = await governanceService.scanForPII('large-dataset', largeContent);
  ...
  describe('Integration Tests', () => {
    it('should perform complete dataset validation workflow', async () => {
      ...
      const compliance = await governanceService.validateLicense(sampleDataset.license);
      ...
      // 5. Manifest Creation
      const manifest = await governanceService.createDatasetManifest([sampleDataset]);
