...
export const mockSoulchainServer = {
  telemetry: (data: SoulchainTelemetry): SoulchainResponse => {
    ...
    const participationRatio = data.consensus.activeVoices / data.consensus.participants;
    ...
    if (entropy < 0.2 && participationRatio > 0.7 && data.consensus.consensusRatio > 0.8) {
      health = 'excellent';
    } else if (entropy < 0.4 && participationRatio > 0.5 && data.consensus.consensusRatio > 0.6) {
      health = 'good';
    } else if (entropy < 0.6 && participationRatio > 0.3 && data.consensus.consensusRatio > 0.4) {
      health = 'fair';
    } else {
      health = 'poor';
    }
